function changeTheme(themeName) {
    if (themeName === 'dark-elf') {
        applyBgPattern('spiderweb');
        if (document.getElementById('bg-pattern')) document.getElementById('bg-pattern').value = 'spiderweb';
    } else if (themeName === 'samurai') {
        applyBgPattern('clouds');
        if (document.getElementById('bg-pattern')) document.getElementById('bg-pattern').value = 'waves';
    } else if (themeName === 'Midnights') {
        applyBgPattern('waves');
        if (document.getElementById('bg-pattern')) document.getElementById('bg-pattern').value = 'waves';
    } else if (themeName === 'barbarian') {
        applyBgPattern('axe');
        if (document.getElementById('bg-pattern')) document.getElementById('bg-pattern').value = 'axe';
    } else if (themeName === 'necromancer') {
        applyBgPattern('skull');
        if (document.getElementById('bg-pattern')) document.getElementById('bg-pattern').value = 'skull';
    } else if (themeName === 'paladin') {
        applyBgPattern('radiant');
        if (document.getElementById('bg-pattern')) document.getElementById('bg-pattern').value = 'radiant';
    } else if (themeName === 'mystic') {
        applyBgPattern('eyes');
        if (document.getElementById('bg-pattern')) document.getElementById('bg-pattern').value = 'eyes';
    } else if (themeName === 'moon-druid') {
        applyBgPattern('leaf');
        if (document.getElementById('bg-pattern')) document.getElementById('bg-pattern').value = 'leaf';
    } else if (themeName === 'fighter') {
        applyBgPattern('triangles');
        if (document.getElementById('bg-pattern')) document.getElementById('bg-pattern').value = 'triangles';
    } else if (themeName === 'Monocromático') {
        applyBgPattern('square');
        if (document.getElementById('bg-pattern')) document.getElementById('bg-pattern').value = 'square';
    } else if (themeName === 'Ébano') {
        applyBgPattern('grid');
        if (document.getElementById('bg-pattern')) document.getElementById('bg-pattern').value = 'grid';
    } else if (themeName === 'mage') {
        applyBgPattern('stars');
        if (document.getElementById('bg-pattern')) document.getElementById('bg-pattern').value = 'stars';
    } else if (themeName === 'owl') {
        applyBgPattern('padrão');
        if (document.getElementById('bg-pattern')) document.getElementById('bg-pattern').value = 'padrão';
    } else if (themeName === 'nord') {
        applyBgPattern('snow');
        if (document.getElementById('bg-pattern')) document.getElementById('bg-pattern').value = 'snow';
    } else if (themeName === 'retro') {
        applyBgPattern('dots');
        if (document.getElementById('bg-pattern')) document.getElementById('bg-pattern').value = 'axe';
    } else if (themeName === 'chocolate-sand') {
        applyBgPattern('axe');
        if (document.getElementById('bg-pattern')) document.getElementById('bg-pattern').value = 'leaf';
    } else if (themeName === 'redtv') {
        applyBgPattern('leaf');
        if (document.getElementById('bg-pattern')) document.getElementById('bg-pattern').value = 'triangles';
    } else if (themeName === 'fliper') {
        applyBgPattern('triangles');
        if (document.getElementById('bg-pattern')) document.getElementById('bg-pattern').value = 'hexagons';
    } else if (themeName === 'cyberpunk') {
        applyBgPattern('hexagons');
        if (document.getElementById('bg-pattern')) document.getElementById('bg-pattern').value = '';
    } else if (themeName === 'sunset') {
        applyBgPattern('diamonds');
        if (document.getElementById('bg-pattern')) document.getElementById('bg-pattern').value = 'dots';
    } else {
        applyBgPattern('crosses');
        if (document.getElementById('bg-pattern')) document.getElementById('bg-pattern').value = 'crosses';
    }
    ...
    if (optionEl && selectedEl) {
        selectedEl.innerText = optionEl.innerText + ' ▼';
    }

    const customBtn = document.getElementById('custom-theme-btn');
    if (customBtn) customBtn.style.display = themeName === 'custom' ? 'inline-block' : 'none';

    const bgBtn = document.getElementById('bg-image-btn');
    if (bgBtn) bgBtn.style.display = themeName === 'custom' ? 'inline-block' : 'none';
    ...
    setTimeout(() => {
        document.body.classList.remove('is-animating-theme');
    }, 600);
}

function updateCustomColor(property, value) {
    // NOVO: Verifica pelo atributo 'data-theme' em vez do .value do select antigo
...
function applyAllCustomColors() {
    ...
    Object.entries(map).forEach(([prop, id]) => {
        ...
        if (el) document.body.style.setProperty(prop, el.value);
    });
}

function openThemeModal() {
    syncModalFromPickers();
    updateAllPreviews();
...
function syncPicker(id, value) {
    const hidden = document.getElementById('picker-' + id);
    if (hidden) {
        hidden.value = value;
    }
    const cssProp = '--' + id;
    updateCustomColor(cssProp, value);
    updatePreview(id, value);
}

function syncPickerHeader(id, value) {
    const hidden = document.getElementById('picker-' + id);
    if (hidden) hidden.value = value;
    updateCustomColor('--' + id, value);

    const top = document.getElementById('mpicker-header-top')?.value || '#0a0804';
    const bot = document.getElementById('mpicker-header-bot')?.value || '#1a1510';
    const grad = `linear-gradient(to bottom, ${top}, ${bot})`;
    const prevTop = document.getElementById('prev-header-top');
    const prevBot = document.getElementById('prev-header-bot');
    if (prevTop) prevTop.style.background = grad;
    if (prevBot) prevBot.style.background = grad;
}
...
/* Sync all modal pickers from hidden pickers (on open) */
function syncModalFromPickers() {
    const ids = ['bg', 'bg2', 'bg3', 'panel', 'border', 'border2', 'accent', 'accent2',
        'input-bg', 'input-border', 'text', 'text2', 'text3', 'shadow', 'glow', 'red', 'red2', 'green',
        'header-top', 'header-bot'
    ];
    ids.forEach(id => {
        const hidden = document.getElementById('picker-' + id);
        const modal = document.getElementById('mpicker-' + id);
...
function updateAllPreviews() {
    ...
    ids.forEach(id => {
        const picker = document.getElementById('mpicker-' + id);
        if (picker) updatePreview(id, picker.value);
    });

    const top = document.getElementById('mpicker-header-top')?.value || '#0a0804';
    const bot = document.getElementById('mpicker-header-bot')?.value || '#1a1510';
    const grad = `linear-gradient(to bottom, ${top}, ${bot})`;
    ['prev-header-top', 'prev-header-bot'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.style.background = grad;
    });
}
function applyFontChange(family) {
    if (preview) preview.style.fontFamily = `'${family}', serif`;
...
function applyGlowOpacity(val) {
    const lbl = document.getElementById('glow-opacity-label');
    if (lbl) lbl.textContent = val + '%';
    const glowColor = document.getElementById('mpicker-glow')?.value || '#c8922a';
    const hex = glowColor.replace('#', '');
    const r = parseInt(hex.slice(0, 2), 16),
        g = parseInt(hex.slice(2, 4), 16),
        b = parseInt(hex.slice(4, 6), 16);
    const rgba = `rgba(${r},${g},${b},${(val / 100).toFixed(2)})`;
    document.body.style.setProperty('--glow', rgba);
}
...
function applyBgPattern(pattern) {
    ...
    let st = document.getElementById('__pattern-override__');
    if (!st) {
        st = document.createElement('style');
        st.id = '__pattern-override__';
        document.head.appendChild(st);
    }

    if (pattern === 'none' || !patterns[pattern]) {
        st.textContent = `body { background-image: none !important; }`;
    } else if (pattern === 'dots') {
        st.textContent = `body { background-image: ${patterns.dots} !important; background-size: 20px 20px !important; }`;
    } else if (pattern === 'grid') {
        st.textContent = `body { background-image: ${patterns.grid} !important; background-size: 30px 30px !important; }`;
    } else if (pattern === 'axe') {
        st.textContent = `body { background-image: ${patterns.axe} !important; background-size: 75px 75px !important; }`;
    } else {
        st.textContent = `body { background-image: ${patterns[pattern]} !important; }`;
    }
}

function applyBorderRadius(val) {
    const lbl = document.getElementById('border-radius-label');
    if (lbl) lbl.textContent = val + 'px';
    let st = document.getElementById('__radius-override__');
    if (!st) {
        st = document.createElement('style');
        st.id = '__radius-override__';
        document.head.appendChild(st);
    }
...
function resetCustomTheme() {
    ...
    const grad = `linear-gradient(to bottom, ${defaults['header-top']}, ${defaults['header-bot']})`;
    ['prev-header-top', 'prev-header-bot'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.style.background = grad;
    });
    ...
    const gr = document.getElementById('glow-opacity');
    if (gr) {
        gr.value = 15;
    }
    const lgo = document.getElementById('glow-opacity-label');
    if (lgo) lgo.textContent = '15%';
