/* ==========================================================================
   SPELL SYSTEM
   Gerencia a aba de magias, slots e cálculos de CD/Ataque Mágico.
   ========================================================================== */
  ...
  function buildLevelBlock(level, container) {
    ...
    if (level > 0) {
      ...
      slotsHTML = `
        <div style="display:flex; align-items:center; gap:6px; margin-left:auto;">
          <span style="font-size:10px; color:var(--text3);">Espaços:</span>
          <input type="number" value="${spellSlots[level].total}" min="0" max="9" style="width:36px; text-align:center; font-size:12px;" onchange="setSlotTotal(${level},this.value)" id="slot-total-${level}">
    ...
    panel.innerHTML = `
      <div class="spell-level-header">
        ...
        <div style="font-family:'Cinzel',serif; font-size:12px; letter-spacing:1px; color:var(--text2);">${levelLabel}</div>
        ${slotsHTML}
      </div>
...
function spellRowHTML(level, i, sp) {
  ...
  return `
    <div class="spell-row" id="spell-row-${level}-${i}">
      ${prepCheck}
      <input type="text" value="${escapeHTML(sp.name)}" placeholder="Nome do feitiço..." oninput="updateSpell(${level},${i},this.value)" style="flex:1;">
      <div class="spell-row-del" onclick="removeSpell(${level}, ${i})" title="Remover">×</div>
    </div>`;
}
